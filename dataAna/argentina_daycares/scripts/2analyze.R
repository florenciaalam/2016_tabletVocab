...
print("!!! NEW ! SHOULD BE DONE FOR ALL OTHERS, IT'S NICE TO HAVE MEAN AND SD")
print("pc, mean & SD")
print(cbind(c("Low","Mid"),round(cbind(
aggregate(data $pc ,by=list(data$SES),mean, na.rm=T)$x,
aggregate(data $pc ,by=list(data$SES),sd, na.rm=T)$x),
	3)))
...
library (psych)
getwd()
setwd("~/Dropbox/2016_tabletVocab/dataAna/argentina_daycares")
getwd()
getwd()
getwd
setwd("~/Dropbox/2016_tabletVocab/dataAna/argentina_daycares")
